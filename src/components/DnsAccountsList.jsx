...
function DnsAccountsList() {
  ...
  const handleRemove = async (id) => {
    try {
      ...
      setDnsAccounts(dnsAccounts.filter((dnsAccount) => dnsAccount._id !== id));
    } catch (error) {
      console.error(
    ...
    <div>
      ...
      {dnsAccounts.length > 0 ? (
        <div className="overflow-x-auto">
          <div className="dnsAccounts">
            {dnsAccounts.map((dnsAccount) => (
              <div
                key={dnsAccount._id}
...
export default DnsAccountsList;
