...
function GitAccountsList() {
  ...
  const handleRemove = async (id) => {
    try {
      ...
      await axios.delete(`/api/gitAccounts`, { data: { id } });
      // Оновлення стану для відображення змін без перезавантаження
      setGitAccounts(gitAccounts.filter((gitAccount) => gitAccount._id !== id));
    } catch (error) {
      console.error(
              ...
              >
                ...
                <div className="flex gap-2 w-1/5 justify-end">
                  <Link
                    ...
                    href={`/git-accounts/${gitAccount._id}`}
                  >
                    <Eye />
                  </Link>
                  <button
...
export default GitAccountsList;
