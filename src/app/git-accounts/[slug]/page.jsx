...
function SingleProject({ params: { slug } }) {
  ...
  useEffect(() => {
    const fetchEmail = async () => {
      try {
        const response = await axios.get(`/api/gitAccounts?id=${slug}`);
        console.log(response.data);
        setGitAccount(response.data.gitAccount);
      } catch (error) {
  ...
  return (
    <>
      {gitAccount ? (
        <>
          <AddGitAccountForm projectData={gitAccount} editable={editableForm} />
          {userRole !== "manager" &&
            (!editableForm ? (
              <button
