...
function HostingsList() {
  ...
  const handleRemove = async (id) => {
    try {
      ...
      await axios.delete(`/api/hostings`, { data: { id } });
            ...
            {hostings.map((hosting) => (
              ...
              >
                <div className="w-4/5 flex flex-col sm:flex-row">
                  <span className="w-full sm:w-1/2">{hosting.name}</span>
                  <span className="w-full sm:w-1/2">{hosting.login}</span>
                </div>
